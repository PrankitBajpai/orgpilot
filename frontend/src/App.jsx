...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body { background:var(--bg); color:var(--text); font-family:var(--ft); min-height:100vh; overflow-y:auto; }
  .shell { display:flex; min-height:100vh; }
  .main  { flex:1; min-height:100vh; background:var(--bg); overflow-y:auto; }
  .pw    { padding:28px 32px 80px; max-width:1280px; }
  ...
  .modal { background:var(--bg2); border:1px solid var(--border2); border-radius:var(--radius); padding:26px; width:100%; max-width:480px; max-height:90vh; overflow-y:auto; }
  ...
  .send-result-item { display:flex; align-items:center; justify-content:space-between; padding:6px 10px; background:var(--bg4); border-radius:var(--radius-sm); margin-bottom:5px; font-size:12px; }
  @media (max-width:900px) { .g4{grid-template-columns:1fr 1fr;} .g2{grid-template-columns:1fr;} .pw{padding:18px 14px 60px;} }
            ...
            <tbody>
              {filtered.map(m=>(
                <tr key={m.id}>
                  ...
                  <td><button className="btn bdr" style={{padding:"4px 10px",fontSize:11}} onClick={()=>setConfirmDelete(m)}>Remove</button></td>
                </tr>
              ))}
            </tbody>
          </table>
          <div style={{padding:"10px 16px",borderTop:"1px solid var(--border)",fontSize:11,color:"var(--text3)"}}>{filtered.length} of {members.length} members</div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setConfirmDelete(null)}>
          <div className="modal" style={{maxWidth:380}}>
            ...
            <div style={{fontSize:13,color:"var(--text2)",marginBottom:20,lineHeight:1.6}}>
              <strong style={{color:"var(--text)"}}>{confirmDelete.name}</strong> will be removed from all future email and meeting lists.
            </div>
    ...
    <div>
      ...
      <div className="g2" style={{marginBottom:18}}>
        <div className="card">
          ...
          {emails.length===0 ? (
            <div className="empty" style={{padding:"28px 0"}}>
              <div className="empty-icon">✉</div><div className="empty-title">No emails sent yet</div>
        ...
        <div className="card">
          ...
          {meetings.length===0 ? (
            <div className="empty" style={{padding:"28px 0"}}>
              <div className="empty-icon">◫</div><div className="empty-title">No meetings scheduled</div>
...
/* ═══════════════════════════════════════
   EMAIL AGENT  — fully fixed
═══════════════════════════════════════ */
function EmailAgent({ members, emails, onEmailSent }) {
  ...
  const generate = async () => {
    ...
    setLoading(true); setSendResults(null); setEmail(null); setDoneSteps([]); setAiError(""); setSendError("");
...
IMPORTANT: respond ONLY with a raw JSON object, no markdown, no backticks, start with { end with }.
{"subject":"...","body":"full email body...","recipient_count":${filtered.length},"estimated_open_rate":"XX%"}`;

      const text  = await gemini(prompt);
      const match = text.match(/\{[\s\S]*\}/);
      if (!match) throw new Error("No JSON in AI response");
      setEmail(JSON.parse(match[0]));
    } catch (err) {
  ...
  const sendEmail = async () => {
    setSending(true); setSendError(""); setSendResults(null);
    try {
      const recipients = filtered.map(m=>m.email);
      const data = await api.post("/email/send", {
        topic, department:dept, tone, recipients,
        subject: email.subject,
        body:    email.body,
      });
      setSendResults(data);
      onEmailSent({ subject:email.subject, recipientCount:data.sent, dept });
    } catch (err) {
  ...
  return (
    <div>
      <div className="pt">✉ Email Agent</div>
      <div className="ps">AI drafts and sends real emails to your team members via Resend</div>
      ...
      <div className="g2">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div className="card">
            ...
            <div className="fg">
              <label className="lbl">Email Topic</label>
              <textarea className="ta" placeholder="e.g. Send project deadline reminder to all developers. Deadline is Friday 6 PM." value={topic} onChange={e=>setTopic(e.target.value)} style={{minHeight:80}}/>
            </div>
            <div className="g2">
            ...
            <button className="btn bp" onClick={generate} disabled={loading||sending||!topic.trim()} style={{width:"100%",justifyContent:"center"}}>
              {loading ? <><div className="spin"/>Generating...</> : `⚡ Generate for ${filtered.length} recipients`}
            </button>
            {emails.length>0 && (
              <div style={{marginTop:10,padding:"7px 11px",background:"rgba(62,207,142,0.07)",border:"1px solid rgba(62,207,142,0.15)",borderRadius:"var(--radius-sm)",fontSize:12,color:"var(--success)"}}>
                ✓ {emails.length} email{emails.length!==1?"s":""} sent this session
              </div>
            )}
          </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          {/* Generated email preview + SEND BUTTON */}
          {email && (
            <div className="card">
              <div className="sh">
                <div className="st">📧 Generated Email</div>
                <div style={{display:"flex",gap:5}}>
                  <span className="badge bb">{email.recipient_count} recipients</span>
                  <span className="badge bg2c">{email.estimated_open_rate}</span>
                </div>
              </div>
              ...
              {sendError && <div className="auth-err" style={{marginBottom:10}}>⚠ {sendError}</div>}

              {sendResults ? (
                <div>
                  <div style={{background:"rgba(62,207,142,0.1)",border:"1px solid rgba(62,207,142,0.22)",borderRadius:"var(--radius-sm)",padding:"11px 14px",marginBottom:10}}>
                    <div style={{fontSize:13,color:"var(--success)",fontWeight:600,marginBottom:6}}>✓ Sent {sendResults.sent} of {sendResults.total} emails</div>
                    {sendResults.failed?.length>0 && <div style={{fontSize:12,color:"var(--warning)"}}>⚠ {sendResults.failed.length} failed: {sendResults.failed.join(", ")}</div>}
                  </div>
                  {sendResults.results?.map((r,i)=>(
                    <div key={i} className="send-result-item">
                      <span style={{color:"var(--text2)"}}>{r.email}</span>
                      <span style={{color:r.status==="sent"?"var(--success)":"var(--error)"}}>{r.status==="sent"?"✓ delivered":"✗ failed"}</span>
                    </div>
                  ))}
                  <button className="btn bg" style={{width:"100%",justifyContent:"center",marginTop:10}} onClick={()=>{setEmail(null);setSendResults(null);}}>
                    ✉ Compose New Email
                  </button>
                </div>
              ) : (
                /* ── THE SEND BUTTON ── */
                <button className="btn bp" style={{width:"100%",justifyContent:"center",padding:"12px 16px",fontSize:14}} onClick={sendEmail} disabled={sending||loading}>
                  {sending ? <><div className="spin"/>Sending real emails...</> : `🚀 Send to ${email.recipient_count} members`}
                </button>
              )}
            </div>
          )}
...
function Scheduler({ members, onMeetingScheduled, meetings }) {
  ...
  const attendees = selEmps.length>0 ? members.filter(m=>selEmps.includes(m.id)) : members;
  ...
  const schedule = async () => {
    ...
    setSending(true); setSendError(""); setSendResults(null);
    try {
      const recipientEmails = attendees.map(m=>m.email);
      const data = await api.post("/meetings/invite", {
        title, date, time, context, agenda,
        attendees: recipientEmails,
        attendee_names: attendees.map(m=>({ name:m.name, email:m.email, role:m.role })),
      });
      setSendResults(data);
      onMeetingScheduled({ title, date, time, attendeeCount:attendees.length, agenda });
    } catch (err) {
  ...
  if (members.length===0) return (
    <div>
      ...
      <div className="ps">AI finds best time slots and sends real calendar invites to your team</div>
      <div className="card"><div className="empty">
        <div className="empty-icon">◫</div>
        <div className="empty-title">No team members yet</div>
  ...
  return (
    <div>
      <div className="pt">◫ Meeting Scheduler</div>
      <div className="ps">AI generates agenda and sends real invite emails to your team</div>
      ...
      <div className="g2">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div className="card">
            ...
            <button className="btn bg" style={{width:"100%",justifyContent:"center",marginBottom:12}} onClick={genAgenda} disabled={loading||!title.trim()}>
          ...
          {/* ── SEND BUTTON lives outside the card so it's always visible ── */}
          {sendResults ? (
            <div className="card">
              <div style={{background:"rgba(62,207,142,0.1)",border:"1px solid rgba(62,207,142,0.22)",borderRadius:"var(--radius-sm)",padding:"11px 14px",marginBottom:10}}>
                <div style={{fontSize:13,color:"var(--success)",fontWeight:600,marginBottom:6}}>✓ Invite sent to {sendResults.sent} of {sendResults.total} attendees</div>
                {sendResults.failed?.length>0 && <div style={{fontSize:12,color:"var(--warning)"}}>⚠ {sendResults.failed.length} failed: {sendResults.failed.join(", ")}</div>}
              </div>
              {sendResults.results?.map((r,i)=>(
                <div key={i} className="send-result-item">
                  <span style={{color:"var(--text2)"}}>{r.email}</span>
                  <span style={{color:r.status==="sent"?"var(--success)":"var(--error)"}}>{r.status==="sent"?"✓ invite sent":"✗ failed"}</span>
                </div>
              ))}
              <button className="btn bg" style={{width:"100%",justifyContent:"center",marginTop:10}} onClick={()=>setSendResults(null)}>
            ...
            <button
              ...
              onClick={schedule}
              disabled={sending||loading||!title.trim()}
            >
              {sending ? <><div className="spin"/>Sending invites...</> : `🗓 Schedule & Send Invites to ${attendees.length} attendees`}
            </button>
          )}
        ...
        {/* ── RIGHT COLUMN — attendee picker ── */}
        <div className="card">
          <div className="sh"><div className="st">👥 Select Attendees</div><span className="badge bb">{attendees.length} selected</span></div>
              ...
              return (
                <div key={m.id} onClick={()=>toggleEmp(m.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 11px",borderRadius:"var(--radius-sm)",background:sel?"rgba(79,142,247,0.08)":"var(--bg3)",border:sel?"1px solid rgba(79,142,247,0.25)":"1px solid var(--border)",cursor:"pointer",transition:"all 0.14s"}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:10,color:"var(--text3)",marginTop:1}}>{m.email}</div>
                  </div>
          ...
          <div style={{marginTop:8,fontSize:11,color:"var(--text3)",textAlign:"center"}}>
            {selEmps.length===0?"No selection = invite everyone":`${attendees.length} attendee${attendees.length!==1?"s":""} will get a real invite email`}
          </div>
        </div>
      </div>
    </div>
...
function HRComms({ members, announcements, onAnnouncementSent }) {
  ...
  const generate = async () => {
    ...
    setLoading(true); setSendResults(null); setAiError(""); setSendError("");
    try {
  ...
  const send = async () => {
    ...
    try {
      ...
      const data = await api.post("/email/send", {
        topic:content, department:"All Departments", tone:"Professional", recipients,
        subject:`[HR ${type}] ${content.slice(0,60)}`,
        body:generated,
      });
      setSendResults(data);
      onAnnouncementSent({ title:content.slice(0,50), type, reach:data.sent });
    } catch (err) {
      setSendError(err.message||"Failed to send emails");
    }
    setSending(false);
      ...
      <div className="g2">
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div className="card">
            ...
            <div style={{fontSize:12,color:"var(--text3)",marginBottom:10}}>Will be sent to {members.length} members via real email</div>
            {aiError && <div className="auth-err" style={{marginBottom:10}}>⚠ {aiError} — using fallback template</div>}
          ...
          {generated && (
            <>
              {sendError && <div className="auth-err">⚠ {sendError}</div>}
              {sendResults ? (
                <div className="card">
                  <div style={{background:"rgba(62,207,142,0.1)",border:"1px solid rgba(62,207,142,0.22)",borderRadius:"var(--radius-sm)",padding:"11px 14px",marginBottom:10}}>
                    <div style={{fontSize:13,color:"var(--success)",fontWeight:600,marginBottom:6}}>✓ Sent to {sendResults.sent} of {sendResults.total} members</div>
                    {sendResults.failed?.length>0 && <div style={{fontSize:12,color:"var(--warning)"}}>⚠ Failed: {sendResults.failed.join(", ")}</div>}
                  </div>
                </div>
              ) : (
                <button className="btn bs" style={{width:"100%",justifyContent:"center",padding:"13px 16px",fontSize:14}} onClick={send} disabled={sending||loading}>
                  {sending ? <><div className="spin"/>Sending...</> : `📤 Send to All ${members.length} Members`}
                </button>
              )}
            </>
          )}
...
function Analytics({ members, emails, meetings, announcements }) {
  const deptBreakdown = DEPTS.map(d=>({
    name:d,
    count:members.filter(m=>m.dept===d).length,
    emails:emails.filter(e=>e.dept===d||e.dept==="All Departments").length,
  })).filter(d=>d.count>0);
      ...
      <div className="g2" style={{marginBottom:18}}>
        <div className="card">
          ...
          {deptBreakdown.length===0 ? (
            <div className="empty" style={{padding:"28px 0"}}><div className="empty-icon">🏢</div><div className="empty-title">No data yet</div><div className="empty-sub">Add members to see department analytics.</div></div>
          ) : (
            <table className="tbl">
              <thead><tr><th>Department</th><th>Members</th><th>Emails</th></tr></thead>
              <tbody>{deptBreakdown.map(d=><tr key={d.name}><td style={{color:"var(--text)",fontWeight:500}}>{d.name}</td><td>{d.count}</td><td>{d.emails}</td></tr>)}</tbody>
            </table>
          )}
        </div>
        <div className="card">
          ...
          {emails.length===0&&meetings.length===0&&announcements.length===0 ? (
            <div className="empty" style={{padding:"28px 0"}}><div className="empty-icon">📈</div><div className="empty-title">No activity yet</div><div className="empty-sub">Start sending emails and scheduling meetings to see your activity timeline.</div></div>
          ) : (
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {[...emails.map(e=>({...e,_type:"email"})),...meetings.map(m=>({...m,_type:"meeting"})),...announcements.map(a=>({...a,_type:"hr"}))]
                .sort((a,b)=>b.id-a.id).slice(0,8).map(item=>(
                <div key={item.id+item._type} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 11px",background:"var(--bg3)",borderRadius:"var(--radius-sm)"}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{fontSize:12,fontWeight:500,color:"var(--text)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.subject||item.title}</div>
                    <div style={{fontSize:11,color:"var(--text3)"}}>{item._type==="email"?`${item.recipientCount} recipients`:item._type==="meeting"?`${item.attendeeCount} attendees`:`${item.reach} recipients`}</div>
                  </div>
      ...
      <div className="g2">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div className="card">
            ...
            {[{label:"Team members",value:members.length},{label:"Emails sent",value:emails.length},{label:"Meetings scheduled",value:meetings.length}].map(c=>(
            ...
            <div style={{marginTop:10,padding:"7px 9px",background:"rgba(62,207,142,0.08)",borderRadius:"var(--radius-sm)",border:"1px solid rgba(62,207,142,0.15)"}}>
              <div style={{fontSize:11,color:"var(--success)"}}>✦ Powered by Claude via OrgPilot Backend</div>
            </div>
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  const addMember    = useCallback((m)=>{ const u=[...members,m]; store.setMembers(u); setMembers(u); },[members]);
  const deleteMember = useCallback((id)=>{ const u=members.filter(m=>m.id!==id); store.setMembers(u); setMembers(u); },[members]);
  const onEmailSent  = useCallback((e)=>{ store.addEmail(e); setEmails(store.getEmails()); },[]);
  const onMeetingScheduled = useCallback((m)=>{ store.addMeeting(m); setMeetings(store.getMeetings()); },[]);
  const onAnnouncementSent = useCallback((a)=>{ store.addAnnouncement(a); setAnnouncements(store.getAnnouncements()); },[]);
  ...
  const pages = {
    ...
    email:     <EmailAgent  members={members} emails={emails} onEmailSent={onEmailSent}/>,
